...
for sample = 1:N_samples * 2  % Generate extra to filter invalid cases
    ...
    % --- Randomize H, G, Theta inside the loop  ---
    H = (randn(N, M) + 1j*randn(N, M)) / sqrt(2);
    G = cell(K,1);
    for k = 1:K
        G{k} = (randn(1,N) + 1j*randn(1,N)) / sqrt(2);
...
K_test_values = [0,1,5,10];   % Test under Rayleigh and two Rician levels
...
ylabel('R^2');
title('R² Across Rician K-Factors (Train K=0, After Optimization)');
ylim([0 1]);
grid on;
set(gca, 'FontSize', 12);
